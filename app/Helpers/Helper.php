...
class Helper
{
     ...
     */
    public static function format_mobile_phone($nomor)
    {
        // Hapus spasi, strip, titik
        $nomor = preg_replace('/[\s\-.]/', '', $nomor);

        // Jika diawali dengan '+62', ubah ke '62'
     ...
     * @return string
     */
    public static function generate_text_avatar()
    {
     ...
     * @return string
     */
    public static function avatar_profile($user = null)
    {
        $user = $user ?? auth()->user();
        $path = $user?->employee?->picture_profile;
        if ($path) {
            try {
                return route('secure.file', [
     ...
     * @return bool
     */
    public static function sendNotification($userId, $title, $message, $link = '#')
    {

        $baseUrl = rtrim(env('FIREBASE_DB_URL'), '/');
        $secret = env('FIREBASE_SECRET');

        $url = "{$baseUrl}/notifications/{$userId}.json?auth={$secret}";
